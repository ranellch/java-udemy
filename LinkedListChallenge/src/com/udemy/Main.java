    ...
    private static Song findSong(String songTitle, String albumTitle) {
        for(int i=0; i<albums.size(); i++) {
            ...
            if(thisAlbum.getTitle().equals(albumTitle)) {
                LinkedList<Song> albumSongs = thisAlbum.getSongs();
                ListIterator<Song> albumSongsIterator = albumSongs.listIterator();

                while(albumSongsIterator.hasNext()) {
                    Song thisSong = albumSongsIterator.next();
                    if(thisSong.getTitle().equals(songTitle)) {
                        return thisSong;
                    }
                }
            }
